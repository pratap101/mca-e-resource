...
if(isset($_FILES['uploaded_file'])) {
    ...
    if($_FILES['uploaded_file']['error'] == 0) {
        ...
        $query = "
           INSERT INTO `file` (
                course,sem,sub,name, mime, size, data, created
            )
            VALUES (
                '{$course}','{$sem}','{$sub}','{$name}', '{$mime}', {$size}, '{$data}', NOW()
            )";
...
<input type="submit" value="UPLOAD NOTES" onclick="return Validate();"  name="submit"/>
</form>
</div>
</br></br>
</section>
